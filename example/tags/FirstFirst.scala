package example.factorization.tags.first
...
object TagsLexer extends Lexers with CharRegExps {
  ...
  type Token    = example.factorization.tags.first.Token  // Tokens.
...
object TagsParser extends Parsers with Transformations with Graphs {
  type Token  = example.factorization.tags.first.Token
  type Kind   = example.factorization.tags.first.TokenKind
